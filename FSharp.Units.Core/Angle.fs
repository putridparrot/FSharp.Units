...
module Angle =
    ...
    type deg = // degrees
        ...
        static member toMilliradians(value : float<deg>) = mrad.create(float value * 17.4533) // (1000.0 * Math.PI)/ 180.0)
    ...
    and [<Measure>] grad = // radians
        ...
        static member toMilliradians(value : float<grad>) = mrad.create(float value * 15.708) // 1000.0 * (Math.PI/200.0))
